...
//-------------------------------------------------------------------------
// Type Templates
// Offsets are always from the beginning of the user space.
//-------------------------------------------------------------------------
...
dcl-ds tmpl_testCase_v6 len(26) qualified template;
...
dcl-ds tmpl_testCaseEvent_v6 len(40) qualified template;
  outcome          char(1);
  reserved_1       char(7);
  // runtimeErrorEvent
  offsExcpMsg      int(10);
...
// @ignore-unused
  dcl-ds testCaseV6 likeds(tmpl_testCase_v6) based(pTestCaseV6);
  dcl-ds testCaseEventV6 likeds(tmpl_testCaseEvent_v6) based(pTestCaseEventV6);
  ...
  pTestCaseV6 = usPtr + header.offsTestCases;
  pTestCaseEventV6 = pTestCaseV6 + %size(testCaseV6);
  ...
  dow (pTestCaseResult <> *null);
    ...
    dow (pAbstractTestEvent <> *null);
      ...
      lenTestCaseEntry = createV6TestCase(usPtr: tmpTestCaseResult
                                          : pTestCaseV6: pTestCaseEventV6: abstractTestEvent);
      header.length += lenTestCaseEntry;
      pTestCaseV6 += lenTestCaseEntry;
...
dcl-proc createV6TestCase;
  dcl-pi *N int(10) extproc(*dclcase);
    ...
    pTestCase         pointer value;
    pTestCaseEvent    pointer value;
...
// @ignore-unused
  ...
  offsEntry = pTestCase - usPtr;

  clear testCase;
  testCase.offsNextEntry = offsEntry + %size(testCase);
  testCase.lenEntry = %size(testCase);

  testCase.result = testCaseResult.outcome;
  testCase.reserved_1 = '';
  testCase.numAsserts = testCaseResult.assertCnt;
  testCase.execTime = testCaseResult.exectime;

  // Test case text:
  testCase.offsTestCaseText = offsEntry + %size(testCase);
  ...
  testCase.offsNextEntry += testCase.lenTestCaseText;
  ...
  when (abstractTestEvent.outcome = TEST_CASE_ERROR);
    ...
    // Exception message:
    testCaseEvent.offsExcpMsg = testCase.offsTestCaseText + testCase.lenTestCaseText;
    ...
    testCase.offsNextEntry += testCaseEvent.lenExcpMsg;
    testCase.lenEntry += testCaseEvent.lenExcpMsg;

    // Execution time:
    testCase.execTime = -1;

    // Sender information:
    testCaseEvent.offsSndInf = offsEntry + testCase.lenEntry;
    ...
    testCase.offsNextEntry += sender.lenEntry;
    testCase.lenEntry += sender.lenEntry;

    // Receiver information:
    testCaseEvent.offsRcvInf = offsEntry + testCase.lenEntry;
    ...
    testCase.offsNextEntry += receiver.lenEntry;
    testCase.lenEntry += receiver.lenEntry;
  ...
  when (abstractTestEvent.outcome = TEST_CASE_FAILURE);
    ...
    // Call stack entries:
    testCaseEvent.offsCallStkE = offsEntry + testCase.lenEntry;
    sPtr = usPtr + testCaseEvent.offsCallStkE;
    ...
    dow (Callstack_getNext(testFailureEvent.pCallstk: callstkEnt));
      ...
      testCase.lenEntry += stackEntry.lenEntry;
      testCase.offsNextEntry += stackEntry.lenEntry;
...
//  entry.offsExpected = offsEntry + entry.lenEntry;
  ...
  entryLogValue.lenEntry = %size(tmpl_logValue_v6)
                              + logValue.length;
