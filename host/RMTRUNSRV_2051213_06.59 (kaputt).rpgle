Version 6:59 und 7:19 (kaputt)

**FREE
// ==========================================================================
//  iRPGUnit - Plug-in Adapter.   06:59 und 07:19 (kaputt)
// ==========================================================================
  ...
  dow (pTestCaseResult <> *null);
    ...
    dow (pAbstractTestEvent <> *null);
      ...
      when g_status.version >= version_5;
        lenTestCaseEntry = createV5TestCase(usPtr: tmpTestCaseResult: pTestCase: abstractTestEvent);
        header.length += lenTestCaseEntry;
        pTestCase += lenTestCaseEntry;
      when g_status.version >= version_4;
        lenTestCaseEntry = createV4TestCase(usPtr: tmpTestCaseResult: pTestCase: abstractTestEvent);
        header.length += lenTestCaseEntry;
        pTestCase += lenTestCaseEntry;
      other;
        lenTestCaseEntry = createV3TestCase(usPtr: tmpTestCaseResult: pTestCase: abstractTestEvent);
        header.length += lenTestCaseEntry;
        pTestCase += lenTestCaseEntry;
      endsl;
...
//    testFailureEvent = getLastFailureEvent(testCaseResult);
    ...
    // add 'expected' value
    entry.offsExpected = addLogValue(usPtr: offsEntry: testFailureEvent.logExpected);

    // add 'actual' value
    entry.offsActual = addLogValue(usPtr: offsEntry: testFailureEvent.logActual);
...
//  entry.offsExpected = offsEntry + entry.lenEntry;
  ...
  entry.lenEntry += logExpected.length;
  entry.offsNextEntry += logExpected.length;

  logValueExpected.lenEntry = %size(tmpl_logValue_v5)
                              + logExpected.length;
  logValueExpected.length = logExpected.length;
  logValueExpected.originalLength = logExpected.originalLength;
  logValueExpected.offsValue = pValueExpected - usPtr;

  logValueExpected.offsDataType = offsEntry + entry.lenEntry;
  logValueExpected.lenDataType = %len(logExpected.type);
  ...
  logValueExpected.lenAssertProc = %len(logExpected.assertProc);
