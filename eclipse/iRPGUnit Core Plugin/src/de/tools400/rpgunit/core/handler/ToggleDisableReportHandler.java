...
public class ToggleDisableReportHandler extends AbstractHandler implements IElementUpdater {
    ...
    public Object execute(ExecutionEvent anEvent) throws ExecutionException {
        if (HandlerUtil.getActivePart(anEvent) instanceof RPGUnitView) {
            ...
            Boolean tReportDisabled = tPreferenceStore.getBoolean(Preferences.REPORT_DISABLED);
            tPreferenceStore.setValue(Preferences.REPORT_DISABLED, !tReportDisabled);
        }
        return null;
    }
    ...
    public void updateElement(UIElement anUIElement, Map aMap) {
        IPreferenceStore tPreferenceStore = RPGUnitCorePlugin.getPreferencesStore();
        Boolean tReportDisabled = tPreferenceStore.getBoolean(Preferences.REPORT_DISABLED);
        anUIElement.setChecked(tReportDisabled);
    }
}
