    ...
    public void editSourceMember(IStructuredSelection tSelection) {
        for (Iterator<?> tIterator = tSelection.iterator(); tIterator.hasNext();) {
            ...
            if (tElement instanceof IUnitTestItemWithSourceMember) {
                ...
                if (!tUnitTestItemWithSource.isSourceMemberAvailable()) {
                    MessageDialog.openError(shell, Messages.ERROR, Messages.Cannot_open_source_member_No_source_information_available);
                }

                EditableSourceMember tEditableSourceMember = tUnitTestItemWithSource.getEditableSourceMember();
